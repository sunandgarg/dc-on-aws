...
BEGIN
  ...
  IF _rank IS NULL THEN
    RETURN;
  END IF;
  IF _rank < 1 OR _rank > 5 THEN
    RAISE EXCEPTION 'rank must be 1..5';
  END IF;

  v_sql := format(
    'UPDATE public.%I SET featured_rank = CASE WHEN featured_rank + 1 > 5 THEN NULL ELSE featured_rank + 1 END WHERE featured_rank IS NOT NULL AND featured_rank >= $1',
    _table
  );
