...
DECLARE
  ...
  created record;
BEGIN
  SELECT status, progress, current_step, message, created_article_ids,
  ...
  RAISE NOTICE 'BLOG_FINAL status=% progress=% step=% created=% message=%',
  ...
  IF latest.created_count > 0 THEN
    ...
    RAISE NOTICE 'BLOG_FINAL_ARTICLE title=% slug=% status=% author_id=% has_cover=%',
